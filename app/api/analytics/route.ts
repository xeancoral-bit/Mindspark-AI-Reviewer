...
export async function GET(req: NextRequest) {
  try {
    ...
    const totalTimeSeconds = progressLogs.reduce<number>((acc, curr) => acc + curr.timeSpentSeconds, 0);
    const totalTimeMinutes = Math.round(totalTimeSeconds / 60);

    const totalScore = progressLogs.reduce<number>((acc, curr) => acc + curr.score, 0);
    const totalMaxScore = progressLogs.reduce<number>((acc, curr) => acc + curr.maxScore, 0);
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayXp = dayLogs.reduce<number>((acc, curr) => acc + curr.xpEarned, 0);
      const dayTime = Math.round(dayLogs.reduce<number>((acc, curr) => acc + curr.timeSpentSeconds, 0) / 60);
